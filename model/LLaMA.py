...
class MultiHeadSelfAttention(nn.Module):
    def __init__(self, embed_dim, num_heads, seq_len, dropout_rate= 0.1, use_lora=False, r=8, alpha = 16):
        super().__init__()
        ...
        self.q_projection_layer = LoRALinear(num_heads * head_dim, use_lora=use_lora, r=r, alpha = alpha, bias=False) # nn.Linear(embed_dim, embed_dim) 
        self.k_projection_layer = nn.Linear(num_heads * head_dim, embed_dim, bias=False) 
        self.v_projection_layer = LoRALinear(num_heads * head_dim, use_lora=use_lora, r=r, alpha = alpha, bias=False) #nn.Linear(embed_dim, embed_dim) 
        self.RoPE  = RotaryPositionalEmbedding(seq_len, embed_dim)
        self.final_projection_layer = nn.Linear(num_heads * head_dim, embed_dim, bias=False) 
        self.embed_dim = embed_dim
    ...
    def scaled_dot_product_attention(self, q, k, v, causal_mask=None, padding_mask = None, past_k = None, past_v = None, inference = False): #(batch_size, seq_len_k, d_k) or (batch_size,num_heads, seq_len_k, d_k)
        ...
        # attn_weights = self.dropout(attn_weights) if not inference else attn_weights LLaMA does NOT use dropout
...
class TransformerBlock(nn.Module):
    def __init__(self, embed_dim = 64, num_heads = 2, seq_len = 16, hidden_dim = 128, dropout_rate = 0.1, use_lora=False, r=8, alpha = 16):
        super().__init__()
        self.self_attention_layer = MultiHeadSelfAttention(embed_dim, num_heads, seq_len, dropout_rate=dropout_rate, use_lora=use_lora, r=r, alpha = alpha)
    ...
    def forward(self, decoder_in, past_k_self = None, past_v_self = None, past_k_cross = None, past_v_cross = None, inference = False):
...
class TransformerDecoder(nn.Module):
    def __init__(self, vocab_size, batch_size = 16, num_layers = 6, seq_len = 16, embed_dim = 64, num_heads = 2, hidden_dim = 128, dropout_rate = 0.1, use_lora=False, r=8, alpha = 16):
        ...
        self.decoder_stack = nn.ModuleList([TransformerBlock(embed_dim, num_heads, seq_len, hidden_dim, dropout_rate, use_lora, r, alpha) for i in range(num_layers)])
        self.dropout = nn.Dropout(dropout_rate)
        self.final_projection_layer = nn.Linear(embed_dim, vocab_size)
    def forward(self,decoder_input, inference = False):
        embedded_x = self.embedding_layer(decoder_input)
        batch_size = embedded_x.size(0)
        seq_len = embedded_x.size(1)
        # x = embedded_x + self.pos_encoding(seq_len, batch_size) #sinusidal
        x = self.dropout(embedded_x)
        ...
        for i in range(self.num_layers):
            x,  past_k_self[i], past_v_self[i], past_k_cross[i], past_v_cross[i]= \
                self.decoder_stack[i](x, past_k_self[i], past_v_self[i], past_k_cross[i], past_v_cross[i], inference = inference)
